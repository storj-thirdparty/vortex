		...
		<div>
			<div class="container">
				<div class="row">
					<div class="col-sm-12">
						<div
							class="card border-0 p-4 p-lg-5 mb-5 mt-4"
							@drop.prevent="upload"
							@dragover.prevent
						>
								...
								<table class="table table-striped table-hover">
									...
									<tbody>
										...
										>
											...
											>
												<p
													class="text-small pr-2 mb-0 text-right"
												>
													{{ user.id }}
												</p>
											</th>
											...
											<td
												class="align-middle text-truncate user-email"
											>
												{{ user.email }}
											</td>
											<td class="align-middle">
												{{
													user.createTime | toNiceDate
												}}
											</td>
											<td class="align-middle">
												{{
											...
											<td class="align-middle pr-2">
												<select
													class="custom-select plan-select mb-0"
													v-on:change="
														setPlan(
															user,
															$event.target.value
...
export default {
	...
	methods: {
		...
		async getInfo(user) {
			const { data } = await axios.post(`/api/admin/json/${user.id}`, {
				token: this.token
			});
